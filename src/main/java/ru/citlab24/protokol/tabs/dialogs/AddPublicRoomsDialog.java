...
/**
 * Диалог пакетного добавления/редактирования комнат ДЛЯ ОБЩЕСТВЕННЫХ помещений.
 * — Снизу широкая строка ввода.
 * — Справа «Быстрый выбор (общественный)»; пополняется мгновенно при добавлении.
 ...
 * — Ctrl+клик по быстрым — сразу в левый список.
 * — Шапки слева/справа в один уровень. Разделитель стартует на 80% слева (шире зона редактирования).
 */
public class AddPublicRoomsDialog extends JDialog {
    ...
    public AddPublicRoomsDialog(JFrame parent,
                                java.util.List<String> suggestions,
                                String prefillName,
                                boolean editMode) {
        super(parent, editMode ? "Редактировать (общественный)" : "Добавить комнаты (общественный)", true);
        this.baseSuggestions = (suggestions != null && !suggestions.isEmpty())
                ? suggestions
                : defaultPublicSuggestions();
        this.editMode = editMode;

        Dimension pref = new Dimension(864, 528); // +20%
    ...
    private JComponent buildCenter() {
        ...
        rightScroll.setPreferredSize(new Dimension(220, 200)); // уже, чтобы центр был шире

        JPanel rightWrap = new JPanel(new BorderLayout());
        JPanel rightHeader = new JPanel(new FlowLayout(FlowLayout.LEFT, 6, 6));
        rightHeader.add(new JLabel("Быстрый выбор (общественный)"));
        ...
        SwingUtilities.invokeLater(() -> split.setDividerLocation(1.80)); // шире зона редактирования
    ...
    // ===== дефолтные подсказки для общественных помещений =====
    private static java.util.List<String> defaultPublicSuggestions() {
        return java.util.List.of(
                "Холл", "Вестибюль", "Коридор",
                "Электрощитовая", "Серверная", "Венткамера",
                "Мусорокамера", "Колясочная"
        );
    }
}
