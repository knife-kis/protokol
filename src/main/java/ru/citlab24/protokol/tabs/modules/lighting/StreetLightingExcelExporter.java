...
public final class StreetLightingExcelExporter {
    ...
    public static void appendToWorkbook(List<RowData> rows, Workbook wb) {
        ...
        wb.setRepeatingRowsAndColumns(wb.getSheetIndex(sh), -1, -1, 5, 5); // строка 6
    ...
    private static double[] buildSeries20(Double v1D, Double v5D, Double v10D, Double v20D, long seed) {
        ...
        // 2..4 между 1 и 5 с шумом ±10%
        for (int i = 1; i <= 3; i++) {
            double t = i / 4.0; // 0.25, 0.5, 0.75
            double base = v1 + (v5 - v1) * t;
            a[i] = base * (1.0 + (rnd.nextDouble() * 0.2 - 0.1));
        }

        // 6 = 5 ±10%
        a[5] = v5 * (1.0 + (rnd.nextDouble() * 0.2 - 0.1));

        // 7..9 между 6 и 10 с шумом ±10%
        ...
        // 11..19 между 10 и 20 с шумом ±10%
        ...
        // === МОНОТОННОСТЬ ПОСЛЕ 10-го: нерастущая последовательность справа ===
        // a[10]..a[19] не должны быть больше значения слева.
        a[10] = Math.min(a[10], a[9]);
        for (int idx = 11; idx <= 19; idx++) {
            a[idx] = Math.min(a[idx], a[idx - 1]);
        }
